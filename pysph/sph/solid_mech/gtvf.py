...
def get_particle_array_gtvf(constants=None, **props):
    ...
    # stress (sigma) = p \delta_{ij} + \sigma^i_{ij}
    ...
    # ########################################################
    # ####### Set some particle constants for simulation #####
    # ########################################################
...
class DensityEvolutionUhat(Equation):
...
class DensityEvolutionU(Equation):
    def initialize(self, d_arho, d_idx):
        d_arho[d_idx] = 0.0
...
class VelocityGradient(Equation):
    ...
    def loop(self, d_rho, s_rho, d_idx, s_idx, d_u, d_v, d_w, d_p, d_ads,
             d_gradvel, s_p, s_m, s_u, s_v, s_w, d_au, d_av, d_aw, DWIJ, d_p0,
             XIJ, RIJ, SPH_KERNEL, HIJ):
        vol_j = s_m[s_idx] / s_rho[s_idx]
        i, j, p, idx = declare("int", 4)
        idx = 9 * d_idx
        vij = declare('matrix(9)', 1)
        vij[0] = s_u[s_idx] - d_u[d_idx]
        vij[1] = s_v[s_idx] - d_v[d_idx]
        vij[2] = s_w[s_idx] - d_w[d_idx]
    ...
    def post_loop(self, d_idx, d_ds, d_ads, d_strain_tensor, d_rotation_tensor,
        ...
        for i in range(3):
            for j in range(3):
                ind = 3 * i + j
                d_ads[idx + ind] += (
                    2 * G * eps[ind] + dsomegaT[ind] + omegads[ind])
                if i == j:
...
class GTVFSolidRK2Step(IntegratorStep):
    ...
    def stage1(self, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_rho, d_au, d_av,
               ...
               d_ds, d_ds0, d_ads, d_p, d_sigma, dt):
        dtb2 = 0.5 * dt
        ...
        i, idx, p, j = declare('int', 4)
        idx = d_idx * 9
        for i in range(3):
            for j in range(3):
                p = 3 * i + j
                d_ds[idx + p] = d_ds0[idx + p] + dtb2 * d_ads[idx + p]
                if i == j:
                    d_sigma[idx + p] = - d_p[d_idx] + d_ds[idx + p]
                else:
                    d_sigma[idx + p] = d_ds[idx + p]
    ...
    def stage2(self, d_idx, d_x, d_y, d_z, d_x0, d_y0, d_z0, d_u, d_v, d_w,
        ...
        for i in range(3):
            for j in range(3):
                p = 3 * i + j
                d_ds[idx + p] = d_ds0[idx + p] + dt * d_ads[idx + p]
                if i == j:
                    d_sigma[idx + p] = - d_p[d_idx] + d_ds[idx + p]
                else:
                    d_sigma[idx + p] = d_ds[idx + p]
