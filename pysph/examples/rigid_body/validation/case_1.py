...
def make_cube(lx, ly, lz, dx):
    ...
    x, y, z = np.mgrid[-lx / 2:lx / 2 + dx:dx, -ly / 2:ly / 2 +
                       dx:dx, -lz / 2:lz / 2 + dx:dx]
    x = x.ravel()
    y = y.ravel()
    z = z.ravel()
    return x, y, z
...
class Case1(Application):
    ...
    def create_particles(self):
        dx = 1.0 / 9.0
        x, y, z = make_cube(0.5, 0.5, 0.5, dx)
        m = np.ones_like(x) * dx * dx * self.rho0
        h = np.ones_like(x) * self.hdx * dx
        rad_s = np.ones_like(x) * dx
        body1 = get_particle_array_rigid_body(name='body1', x=x, y=y, z=z, h=h,
                                              m=m, rad_s=rad_s)
        body1.vc[0] = 2.
        ...
        h = np.ones_like(x) * self.hdx * dx
        rad_s = np.ones_like(x) * dx
        body2 = get_particle_array_rigid_body(name='body2', x=x, y=y, z=z, h=h,
                                              m=m, rad_s=rad_s)
        body2.vc[0] = -2.
