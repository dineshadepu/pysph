...
def post_process(case):
    ...
    for sd, array in iter_output(files[1:], 'body'):
        ...
        # rotational energy
        rot_energy = 0.5 * np.dot(array.omega, np.matmul(I, array.omega))
        trans_energy = 0.5 * array.total_mass[0] * np.linalg.norm(array.vc)**2.
        pot_energy = abs(array.total_mass[0] * 9.81 * array.cm[2])

        total_energy = rot_energy + trans_energy + pot_energy
    ...
    plt.legend()
    plt.savefig("case_0_ang_mom", dpi=300)
    ...
    plt.legend()
    plt.savefig("case_0_lin_mom", dpi=300)
    ...
    plt.savefig("case_0_energy", dpi=300)
