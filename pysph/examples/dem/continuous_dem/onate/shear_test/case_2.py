...
class ShearTest(Application):
    ...
    def create_particles(self):
        x, y = np.array(
            [0., 2. * self.radius, 4. * self.radius,
             6. * self.radius]), np.array([0., 0., 0., 0.])
        ...
        eq1 = [
            Group(equations=[
                ApplyTensionForce(dest='beam', sources=None, fx=np.array(
                    [0.]), fy=np.array([self.frc]), fz=np.array(
                        [0.]), idx=np.array([3]))]),
            Group(equations=[
            ...
            Group(equations=[
                GlobalDampingForce(dest='beam', sources=None, alpha_t=self.alpha_t)
            ]),
            Group(equations=[
                ResetForce(dest='beam', sources=None, idx=np.array([0]))
            ])
        ]
        eq2 = [
            Group(equations=[
