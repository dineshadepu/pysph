...
from pysph.dem.continuous_dem.potyondy_ligghts import (
    ...
    setup_bc_contacts_from_limit, BodyForce, DampingForce,
...
class TensionTest(Application):
    ...
    def create_particles(self):
        ...
        beam = get_particle_array_bonded_dem_potyondy_ligghts(
            ...
            rad_s=self.dx / 2., dem_id=1, h=1.2 * self.dx, name="beam")
    ...
    def create_equations(self):
        eq1 = [
            Group(equations=[
                ...
                PotyondyLigghtsIPForceStage1(dest='beam', sources=None,
                                             kn=self.kn, dim=2),
        ...
        eq2 = [
            Group(equations=[
                ...
                # ApplyTensionForce(dest='beam', sources=None, fx=50., idx=self.idx),
                ApplyShearForce(dest='beam', sources=None, fy=50., idx=self.idx),
                PotyondyLigghtsIPForceStage2(dest='beam', sources=None,
                                             kn=self.kn, dim=2),
                ResetForce(dest='beam', sources=None, x=self.dx),
                # DampingForce(dest='beam', sources=None, alpha=0.7)
            ])
