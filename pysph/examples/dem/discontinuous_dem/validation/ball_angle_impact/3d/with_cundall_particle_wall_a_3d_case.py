...
class BouncingBall(Application):
    ...
    def create_particles(self):
        ...
        sand = get_particle_array_dem_3d_linear_cundall(
            x=xp, y=yp, z=zp, v=1., m=m, I_inverse=I_inverse,
            m_inverse=m_inverse, rad_s=self.radius, dem_id=1,
            h=1.2 * self.radius, name="sand")
        ...
        nyw_a = np.array([0.])
        nzw_a = np.array([1.])
        wall = get_particle_array(x=xw_a, y=yw_a, z=zw_a, nx=nxw_a, ny=nyw_a,
                                  nz=nzw_a, constants={'np':
                                                       len(xw_a)}, name="wall")
    ...
    def create_scheme(self):
        dem3drk2 = Dem3dCundallScheme(
            ...
            kn=self.kn, mu=self.mu, en=self.en, gz=-9.81, integrator="rk2")
        dem3deuler = Dem3dCundallScheme(
            bodies=['sand'], solids=None, walls=['wall'], dim=self.dim,
            kn=self.kn, mu=self.mu, en=self.en, gz=-9.81, integrator="euler")
