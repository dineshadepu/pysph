...
class TestBoundaryParticlesWithNormalNorm():
    ...
    def create_equations_single_fluid(self, fluid_name):
        ...
        eqs = [
            ...
            Group(equations=[
                IdentifyBoundaryParticle1(dest=dest, sources=sources,
                                          fac=self.fac)
            ]),
            Group(equations=[
                IdentifyBoundaryParticle2(dest=dest, sources=sources,
                                          fac=self.fac)
            ]),
        ]
        return eqs
    ...
    def create_equations_single_fluid_with_boundary(self, fluid_name,
        ...
        eqs = [
            ...
            Group(equations=[
                IdentifyBoundaryParticle1(dest=dest, sources=[fluid_name],
                                          fac=self.fac)
            ]),
            Group(equations=[
                IdentifyBoundaryParticle2(dest=dest, sources=[fluid_name],
                                          fac=self.fac)
            ]),
        ]
        return eqs
    ...
    def test_dam_break_geometry(self):
        ...
        fluid = get_particle_array_gtvf(name='fluid', x=xf, y=yf, h=h, m=m,
                                        rho=rho)
        boundary = get_particle_array_gtvf(name='boundary', x=xb, y=yb, h=h,
                                           m=m, rho=rho)
        add_ctvf_properties(fluid)
...
class TestBoundaryParticlesWithCosAngle():
    ...
    def create_equations_single_fluid(self, fluid_name):
        ...
        eqs = [
            ...
            Group(equations=[
                IdentifyBoundaryParticleCosAngle(
                    dest=dest, sources=[fluid_name])
            ]),
            # Group(equations=[IdentifyBoundaryParticle2(dest=dest, sources=sources, fac=self.fac)]),

            Group(equations=[SetHIJForInsideParticles(dest=dest, sources=[fluid_name], h=self.h, kernel_factor=self.kernel_factor)]),
        ]
        return eqs
    ...
    def test_circle_geometry(self):
        self.folder_name = "test_circle_geometry_cos_angle_output"
    ...
    def test_sin_geometry(self):
        self.folder_name = "test_sin_geometry_cos_angle_output"
    ...
    def test_dam_break_geometry(self):
        self.folder_name = "test_dam_break_geometry_cos_angle_output"
        self.dim = 2
        dx = 0.03
        self.fac = dx / 2
        self.kernel_factor = 3
    ...
    def test_dam_break_geometry_later(self):
        self.folder_name = "test_dam_break_geometry_later_cos_angle_output"
        self.dim = 2
        dx = 0.03
        self.fac = dx / 2
        self.kernel_factor = 3
...
if __name__ == '__main__':
    # test = TestBoundaryParticlesWithNormalNorm()
