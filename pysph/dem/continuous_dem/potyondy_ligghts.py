...
class Potyondy3dIPForceStage1(Equation):
    ...
    def initialize(self, d_idx, d_bc_total_contacts, d_x, d_y, d_z, d_bc_limit,
        ...
        for i in range(p, q1):
            ...
            d_bc_ft_x[i] = dissipate * d_bc_tort_x[i] + dttorque[0]
            d_bc_ft_y[i] = dissipate * d_bc_tort_y[i] + dttorque[1]
            d_bc_ft_z[i] = dissipate * d_bc_tort_z[i] + dttorque[2]
            ...
            d_torx[d_idx] += d_rad_s[d_idx] * tor1 + (
                d_bc_torn_x[i] + d_bc_tort_x[i])
            d_tory[d_idx] += d_rad_s[d_idx] * tor2 + (
                d_bc_torn_y[i] + d_bc_tort_y[i])
            d_torz[d_idx] += d_rad_s[d_idx] * tor3 + (
                d_bc_torn_z[i] + d_bc_tort_z[i])
...
class RK2StepPotyondy3d(IntegratorStep):
    def initialize(self, d_idx, d_x, d_y, d_z, d_x0, d_y0, d_z0, d_u, d_v, d_w,
                   ...
                   d_bc_ft_z, d_bc_ft0_x, d_bc_ft0_y, d_bc_ft0_z, d_bc_fn_x,
                   d_bc_fn_y, d_bc_fn_z, d_bc_fn0_x, d_bc_fn0_y, d_bc_fn0_z,
                   d_bc_tors_x, d_bc_tors_y, d_bc_tors_z, d_bc_tors0_x,
                   d_bc_tors0_y, d_bc_tors0_z, d_bc_torn_x, d_bc_torn_y,
        ...
        for i in range(p, q):
            ...
            d_bc_tors0_x[i] = d_bc_tors_x[i]
            d_bc_tors0_y[i] = d_bc_tors_y[i]
            d_bc_tors0_z[i] = d_bc_tors_z[i]
