...
def get_particle_array_dem_nonlinear(constants=None, **props):
...
class NonLinearParticleParticleForceStage1(Equation):
    """Force between two spheres is implemented using Nonlinear DEM contact force
    law.

    The force is modelled from reference [1].

    [1] An experimental and numerical study of the angle of repose of coarse
    spheres
    """

    def __init__(self, dest, sources, dt, en=0.1, mu=0.1):
        """the required coefficients for force calculation.
        ...
        self.mu = mu
        self.dtb2 = dt / 2.
        super(NonLinearParticleParticleForceStage1, self).__init__(
            dest, sources)
    ...
    def loop(self, d_idx, d_m, d_fx, d_fy, d_fz, d_tng_x, d_tng_y, d_tng_z,
        ...
        if overlap > 0:
            ...
            tmp = 6 * m_eff * E_eff * sqrt(r_eff * overlap)
            eta_n = self.cn * sqrt(tmp)
            ...
            d_tng_x[found_at] += vt_x * self.dtb2
            d_tng_y[found_at] += vt_y * self.dtb2
            d_tng_z[found_at] += vt_z * self.dtb2
...
class NonLinearParticleParticleForceStage2(Equation):
    """Force between two spheres is implemented using Nonlinear DEM contact force
    law.
    ...
    def __init__(self, dest, sources, en=0.1, mu=0.1):
        ...
        self.alpha_2 = -log_et * sqrt(5. / (log_et**2. + pi**2.))
        self.mu = mu
        super(NonLinearParticleParticleForceStage2, self).__init__(
            dest, sources)
    ...
    def loop(self, d_idx, d_m, d_fx, d_fy, d_fz, d_tng_x, d_tng_y, d_tng_z,
        ...
        if overlap > 0:
            ...
            if found == 1:
                ...
                # increment the tang spring to next time step
                # which here is stage 2
                d_tng_x[found_at] = d_tng_x0[found_at] + vt_x * self.dtb2
                d_tng_y[found_at] = d_tng_y0[found_at] + vt_y * self.dtb2
                d_tng_z[found_at] = d_tng_z0[found_at] + vt_z * self.dtb2
