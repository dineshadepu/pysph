...
class LinearPPFDEMStage1(Equation):
    ...
    def loop(self, d_idx, d_m, d_fx, d_fy, d_fz, d_tng_x, d_tng_y, d_tng_z,
        ...
        if overlap > 0:
            ...
            m_eff = d_m[d_idx]
            eta_n = self.alpha_1 * sqrt(m_eff * self.kn) * overlap**0.25
            ...
            # compute the damping constants
            m_eff_t = 2. / 7. * m_eff
            eta_t = self.alpha_2 * sqrt(m_eff_t * self.kt)
...
class LinearPPFDEMStage2(Equation):
    ...
    def loop(self, d_idx, d_m, d_fx, d_fy, d_fz, d_tng_x, d_tng_y, d_tng_z,
             ...
             d_total_tng_contacts, d_dem_id, d_limit, d_vtx, d_vty, d_vtz,
             d_tng_nx, d_tng_ny, d_tng_nz, d_tng_nx0, d_tng_ny0, d_tng_nz0,
             VIJ, XIJ, RIJ, d_rad_s, d_wx, d_wy, d_wz, d_torx, d_tory, d_torz,
             s_idx, s_rad_s, s_m, s_dem_id, s_wx, s_wy, s_wz, dt):
        ...
        if overlap <= 0:
            if found == 1:
                # make its tangential displacement to be at time
                # t, don't increment it.
                d_tng_nx[found_at] = d_tng_nx0[found_at]
                d_tng_ny[found_at] = d_tng_ny0[found_at]
                d_tng_nz[found_at] = d_tng_nz0[found_at]

                d_tng_x[found_at] = d_tng_x0[found_at]
                d_tng_y[found_at] = d_tng_y0[found_at]
                d_tng_z[found_at] = d_tng_z0[found_at]
        ...
        # if particles are in contact
            ...
            log_en = log(self.en)
            eta_n = -2. * log_en * sqrt(
                m_eff * self.kn) / sqrt(pi**2. + log_en**2.)
            ...
            log_et = log(self.et)
            eta_t = -2. * log_en * sqrt(
                m_eff_t * self.kn) / sqrt(pi**2. + log_et**2.)
            ...
            d_tng_y[found_at] = d_tng_x0[found_at] + vt_y * dt
            d_tng_z[found_at] = d_tng_x0[found_at] + vt_z * dt
